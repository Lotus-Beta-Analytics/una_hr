...
class RatioAnalysisReportHandler(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        # Ratios list: (Name, Value, FormatType)
        # format_type = "ratio" | "percent" | "number"
        ratios = [
            ('Current Ratio', self._compute_current_ratio(options), "ratio"),
            ('Return on Equity', self._compute_return_on_equity(options), "percent"),
            ('Return on Assets', self._compute_return_on_assets(options), "percent"),
            ('EBIT', self._compute_ebit(options), "number"),
            ('EBITDAR', self._compute_ebitdar(options), "number"),
            ('Gross Profit Margin', self._compute_gross_profit_margin(options), "percent"),
            ('Net Profit Margin', self._compute_net_profit_margin(options), "percent"),
            ('Direct Cost Margin', self._compute_direct_cost_margin(options), "percent"),
            ('Other Operating Cost Margin', self._compute_other_operating_margin(options), "percent"),
            ('Interest Coverage Ratio', self._compute_interest_coverage(options), "ratio"),
            ('Assets Turnover', self._compute_assets_turnover(options), "number"),
            ('Debt Ratio', self._compute_debt_ratio(options), "percent"),
        ]

        for name, value, fmt in ratios:
            display_value = self._format_value(value, fmt)
            lines.append({
                'id': name.replace(" ", "_"),
                'name': name,
    ...
    def _get_account_balance(self, options, tags):
        """Dummy placeholder. Replace with real queries/mappings."""
        return 100000.0
